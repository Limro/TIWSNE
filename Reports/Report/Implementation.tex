...
For transfer of the data, the TelosB radio framework is used. 
...
This is done since the normal application of a wireless sensor sends a fix size measurement. 
Furthermore is the packet size relatively small, since a measurement point often generates a small amount of data. 
For the image transfer application the image data is large and the data size can vary between compressions.

A series of design steps is therefore taken to fit the very flexible application needed to the more static platform provided. 
To fit the dynamic message to to the fix size architecture the data split into smaller sizes of maximum 50 bytes. 
...
When chopping the file in smaller pieces, it is important to be able to reconstruct the image on the receiver side, and detect if a packet is lost. 
...
\fxnote{Are the descriptions of the algorithms in the following sub sections necessary? I think the figures are enough.}
