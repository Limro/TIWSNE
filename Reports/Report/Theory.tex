...
Energy consumption is important when we look at wireless sensor networks. The component with the most energy consumption of the TelosB is the Radio circuitry as seen in the TelosB datasheet\fxnote{Reference to the datasheet. Put it in the datasheet folder on CD}. The challenge is to choose when to use the radio and how often. When comparing the radio current (23 mA) with the processing current (1.8 mA), it is clear that it is better to process data before sending it. Processing data in order to reduce package size or transmission rate is the basis principle of this mini project. 
